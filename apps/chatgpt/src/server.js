...
const DATA_DIR = path.resolve(
  process.env.GHOSTWRITER_DATA_DIR ?? path.join(REPO_ROOT, "data"),
);
const PREVIEW_URI = "ui://ghostwriter/carousel-preview-v1.html";
const previewHtml = fs.readFileSync(
  path.join(APP_DIR, "assets", "carousel-preview.html"),
  "utf8",
);
...
function createMcpServer() {
  ...
  server.registerTool(
    "get_identity",
    {
      ...
      description:
        "Read the creator/brand identity Ghostwriter uses for social content generation.",
      inputSchema: {},
      annotations: {
        readOnlyHint: true,
    ...
    async () => {
      ...
      return textResult(
        identity
          ? `Loaded Ghostwriter identity for ${identity.name}.`
          : "No Ghostwriter identity has been configured yet.",
        { identity },
      );
    },
  );
  ...
  server.registerTool(
    "save_identity",
    {
      ...
      description:
        "Create or replace the creator identity, visual rules, voice, audience, content pillars, CTA rules, and hashtag limit used by Ghostwriter.",
      inputSchema: {
        name: z.string().min(1),
        visual: z
          .object({
    ...
    async (input) => {
      ...
      return textResult(`Saved Ghostwriter identity for ${identity.name}.`, {
        identity,
      });
    },
  );
  ...
  server.registerTool(
    "generate_carousel",
    {
      ...
      description:
        "Generate and persist a new Instagram carousel draft from the saved Ghostwriter identity and strategy. Use when the user asks Ghostwriter to create, draft, or prepare a carousel.",
      inputSchema: {
        objective: z
          .string()
          .default("engagement")
          .describe(
            "Primary objective, e.g. engagement, saves, shares, follows, education, conversion.",
          ),
        pillar: z
          .string()
          .optional()
          .describe(
            "Optional content pillar. If omitted, Ghostwriter chooses using performance-aware strategy weights.",
          ),
      },
      annotations: {
        readOnlyHint: false,
      ...
      _meta: {
        ...
        "openai/toolInvocation/invoking":
          "Ghostwriter is drafting the carousel…",
  ...
  server.registerTool(
    "get_draft",
    {
      ...
      description:
        "Read one previously generated Ghostwriter carousel draft by ID.",
    ...
    async ({ draftId }) => {
      ...
      if (!draft)
        return textResult(`No draft found with ID ${draftId}.`, {
          draft: null,
        });
  ...
  server.registerTool(
    "get_history",
    {
      ...
      description:
        "Read Ghostwriter publishing history and stored post metrics used by the strategy engine.",
      inputSchema: {},
      annotations: {
        readOnlyHint: true,
        destructiveHint: false,
    ...
    async () => {
      ...
      return textResult(
        `Ghostwriter has ${history.length} history entr${history.length === 1 ? "y" : "ies"}.`,
        { history },
      );
    },
  );
  ...
  server.registerTool(
    "record_metrics",
    {
      ...
      description:
        "Store performance metrics for a published Ghostwriter post so future pillar selection can learn from results.",
      inputSchema: {
    ...
    async ({ mediaId, metrics }) => {
      ...
      return textResult(`Updated metrics for Instagram media ${mediaId}.`, {
        entry,
      });
    },
  );
  ...
  server.registerTool(
    "publish_carousel",
    {
      ...
      description:
        "Publish an existing Ghostwriter draft to the configured Instagram professional account using the Meta Graph API. Requires 2-10 publicly reachable image URLs. This is an external write action and creates a real Instagram post.",
      inputSchema: {
    ...
    async ({ draftId, imageUrls }) => {
      ...
      return textResult(
        `Published Ghostwriter draft ${draftId} to Instagram as media ${post.mediaId}.`,
        { post },
      );
    },
  );
  ...
  registerAppResource(
    server,
    "Ghostwriter Carousel Preview",
    PREVIEW_URI,
    {
      mimeType: RESOURCE_MIME_TYPE,
      description:
        "Compact preview of a generated Ghostwriter Instagram carousel.",
    },
    async () => ({
      contents: [
        {
          ...
          _meta: {
            "openai/widgetDescription":
              "Shows the generated Ghostwriter carousel title, pillar, hook, slide copy, and draft status.",
          },
        },
      ],
    }),
...
app.use("/mcp", (req, res, next) => {
  const token = process.env.GHOSTWRITER_ADMIN_TOKEN;
...
app.get("/health", (_req, res) =>
  res.json({ ok: true, name: "ghostwriter-chatgpt-app", version: VERSION }),
);
app.all("/mcp", async (req, res) => {
  ...
  try {
    ...
    console.error("Ghostwriter MCP request failed");
    if (!res.headersSent) {
      res
        .status(500)
        .json({
...
const listener = app.listen(PORT, () => {
  console.log(
    `Ghostwriter ChatGPT app listening on http://localhost:${listener.address().port}/mcp`,
  );
});
